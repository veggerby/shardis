...
public sealed class InMemoryShardMap(IReadOnlyDictionary<ShardId, string> map) : IShardMap
{
    private readonly IReadOnlyDictionary<ShardId, string> _map = map?.Count == 0 ? Array.Empty<KeyValuePair<ShardId, string>>().ToDictionary(k => k.Key, v => v.Value) : new Dictionary<ShardId, string>(map!);
