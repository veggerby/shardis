...
public sealed class EfCoreMetricsTests
{
    ...
    private static PersonContext Create(int shard)
    {
        ...
        if (!ctx.People.Any())
        {
            ctx.People.AddRange(new Person { Id = shard * 10 + 1, Age = 30 }, new Person { Id = shard * 10 + 2, Age = 10 });
            ctx.SaveChanges();
        }
        return ctx;
    }
    ...
    private sealed class PersonContext : DbContext { public PersonContext(DbContextOptions<PersonContext> o) : base(o) { } public DbSet<Person> People => Set<Person>(); }
